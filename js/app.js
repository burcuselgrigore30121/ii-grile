...
function courseOrder(course){const m=String(course).match(/Curs\s+(\d+)/);return m?Number(m[1]):99}
const COURSES=[...new Set(QUESTION_BANK.map(q=>q.course))].sort((a,b)=>courseOrder(a)-courseOrder(b)||a.localeCompare(b,'ro'));
